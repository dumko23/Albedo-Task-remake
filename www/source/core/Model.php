...
class Model
{
    ...
    public function validation($config, $record): bool|string
    {
        ...
        foreach ($this->rules() as $fieldName => $rule) {
            ...
            } elseif (str_contains($rule, 'invalid')) {
            ...
            } elseif (str_contains($rule, 'emailFormat') && filter_var($record[$fieldName], FILTER_VALIDATE_EMAIL) === false) {
                $errors = $this->addError($errors, $fieldName, 'Incorrect email format!');
            } elseif (str_contains($rule, 'unique') && isset(Application::get('database')->searchInDB(
                'memberId',
            ...
            } elseif (str_contains($rule, 'country')) {
