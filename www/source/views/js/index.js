...
function sendData (n) {
  const oldForm = document.forms.form
  const formData = new FormData(oldForm)
  const file_data = $('#imgLoad').prop('files')[0]
...
function updateData (n) {
  ...
  const fileUploaded = document.getElementById('imgLoad').files[0]
  if (fileUploaded) {
    if (fileUploaded.size > 10485760) {
      const fileSize = fileUploaded.size / 1048576
...
function updateSize () {
  const file = document.getElementById('imgLoad').files[0]
